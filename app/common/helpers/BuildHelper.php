...
//         var_dump($options);exit;
    ...
    public static function buildFilter($models, $className, $primaryKey = 'id', $pidKey = 'pid', $nameKey = 'name', $isTop = true, $type = null, $searchName = '', $feild = '')
    {
        $models = BuildHelper::reBuildModelKeys($models, $primaryKey);//重构模型数组索引
        $nexus = self::getModelNexus($models, $className, $primaryKey, $pidKey);//获取父子关系
        $list = self::buildList($nexus);
        ...
        foreach ($list as $id => $item) {
            ...
            if(isset($models[$id]) && (is_null($type) || $models[$id]->type == $type)) {
                $str .= '<a href="'.Url::to(['index', $searchName.'['.$feild.']' => $id]).'">'.str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;', $item['level']-1).(empty($item['level']-1)?'':'|-').($models[$id]->{$nameKey}).'</a>';
            }
        }
