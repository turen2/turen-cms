...
/* @var $model backend\models\ext\Nav */
...
<?php $form = ActiveForm::begin([
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="nav-form form-table">
    	<tr>
    		...
    		<td class="second-column">
    			<?= Html::activeDropDownList($model, 'parentid', BuildHelper::buildSelectorData(Nav::find()->current()->orderBy(['orderid' => SORT_DESC])->all(), Nav::class, 'id', 'parentid', 'menuname'), ['encode' => false, 'class' => '']) ?>
    			<span class="cnote">带<span class="maroon">*</span>号表示为必填项</span>
    		</td>
    	</tr>
    	<tr>
    		<td class="first-column"><?= $model->getAttributeLabel('menuname')?><?php if($model->isAttributeRequired('menuname')) { ?><span class="maroon">*</span><?php } ?></td>
    		<td class="second-column">
    			<?= Html::activeInput('text', $model, 'menuname', ['class' => 'input']) ?>
    	...
    	<tr>
    		...
    		<td class="second-column">
    			...
    			<span class="cnote"><span class="gray-btn" onclick="">链接</span></span>
    		</td>
    	</tr>
    	<tr>
    	...
    	<tr>
    		<td class="first-column"><?= $model->getAttributeLabel('target')?><?php if($model->isAttributeRequired('target')) { ?><span class="maroon">*</span><?php } ?></td>
    		<td class="second-column">
    			<?= Html::activeInput('text', $model, 'target', ['class' => 'inputs']) ?>
    			<select name="seltarget" id="seltarget" onchange="$('#nav-target').val($(this).val());">
