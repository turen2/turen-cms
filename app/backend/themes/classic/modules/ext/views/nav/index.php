...
<div rel="rowpid_<?=$model->getTopID('parentstr', 1)?>">
    ...
    <tbody>
    	<tr align="left" class="data-tr">
			...
			<td align="center">
				<a href="<?=Url::to(['move', 'type' => Nav::ORDER_UP_TYPE, 'kid' => $model->id, 'pid' => $model->parentid, 'orderid' => $model->orderid])?>" class="left-arrow" title="提升排序"></a>
				<input type="text" name="orderid[]" id="orderid[]" class="inputls" value="<?= $model->orderid; ?>">
				<a href="<?=Url::to(['move', 'type' => Nav::ORDER_DOWN_TYPE, 'kid' => $model->id, 'pid' => $model->parentid, 'orderid' => $model->orderid])?>" class="right-arrow" title="下降排序"></a>
			</td>
