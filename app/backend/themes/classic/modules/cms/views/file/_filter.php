...
$buildFilter = BuildHelper::buildFilter(Column::find()->current()->orderBy(['orderid' => SORT_DESC])->all(), Column::class, 'id', 'parentid', 'cname', true, $type, 'FileSearch', 'columnid');
...
if(Yii::$app->params['config_init_open_cate']) {//是否开启分类
    ...
    $buildFilter = BuildHelper::buildFilter(Cate::find()->current()->orderBy(['orderid' => SORT_DESC])->all(), Cate::class, 'id', 'parentid', 'catename', true, null, 'FileSearch', 'cateid');
    if(!empty($buildFilter)) {
