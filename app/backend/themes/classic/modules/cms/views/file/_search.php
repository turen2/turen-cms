...
/* @var $model backend\models\cms\FileSearch */
...
<div class="article-search toolbar-tab">
	<ul class="fl">
        ...
        <li class="line">-</li>
        <li class="<?= (!is_null($model->status) && $model->status == File::STATUS_ON)?'on':''?>"><?= Html::a('显示', ArrayHelper::merge(['index', Html::getInputName($model, 'status') => File::STATUS_ON], $addRoutes)) ?></li>
        <li class="line">-</li>
        <li class="<?= (!is_null($model->status) && $model->status == File::STATUS_OFF)?'on':''?>"><?= Html::a('隐藏', ArrayHelper::merge(['index', Html::getInputName($model, 'status') => File::STATUS_OFF], $addRoutes)) ?></li>
        <li class="line">-</li>
        
        <?php foreach (Flag::FlagList(Column::COLUMN_TYPE_FILE, true) as $key => $name) { ?>
