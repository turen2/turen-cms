...
$buildFilter = BuildHelper::buildFilter(Column::find()->current()->orderBy(['orderid' => SORT_DESC])->all(), Column::class, 'id', 'parentid', 'cname', true, $type, 'PhotoSearch', 'columnid', ArrayHelper::merge($addRoutes, [Html::getInputName($model, 'cateid') => $model->cateid]));
...
if(Yii::$app->params['config.openCate']) {//是否开启分类
    ...
    $buildFilter = BuildHelper::buildFilter(Cate::find()->current()->orderBy(['orderid' => SORT_DESC])->all(), Cate::class, 'id', 'parentid', 'catename', true, null, 'PhotoSearch', 'cateid', ArrayHelper::merge($addRoutes, [Html::getInputName($model, 'columnid') => $model->columnid]));
    if(!empty($buildFilter)) {
