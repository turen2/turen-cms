    	...
    	<tr align="left" class="data-tr">
    		...
    		<td width="40%">
    			...
    			<?php } else { ?>
    				...
    				<span class="sub-type"><?= $model->cname ?></span>
    			<?php } ?>
    		</td>
    		<td width="20%" align="center">
    			<a href="<?=Url::to(['move', 'type' => ProductCate::ORDER_UP_TYPE, 'id' => $model->id, 'pid' => $model->parentid, 'orderid' => $model->orderid])?>" class="left-arrow" title="提升排序"></a>
    			<input type="text" name="orderid[]" id="orderid[]" class="inputls" value="<?= $model->orderid; ?>">
    			<a href="<?=Url::to(['move', 'type' => ProductCate::ORDER_DOWN_TYPE, 'id' => $model->id, 'pid' => $model->parentid, 'orderid' => $model->orderid])?>" class="right-arrow" title="下降排序"></a>
    		</td>
    		<td width="32%" class="action end-column">
    			<span><a href="<?= Url::to(['create', 'pid' => $model->id]) ?>">添加子分类</a></span> | 
    			<span><a href="<?= Url::to(['/shop/attribute/index', 'AttributeSearch[pcateid]' => $model->id]) ?>">属性管理</a></span> | 
    			<span><?= $checkstr; ?></span> | 
...
<div class="bottom-toolbar clearfix">
	...
	<?= Html::a('添加新产品分类', ['create'], ['class' => 'data-btn']) ?>
</div>
