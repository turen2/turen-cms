...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="data-table">
	...
	<?php foreach ($dataProvider->getModels() as $key => $model)
	{
		...
		$options = [
		    ...
		    'onclick' => 'turen.com.deleteItem(this, \''.$model->catename.'\')',
		];
    		...
    		<tbody>
    			<tr align="left" class="data-tr">
    				...
    				<td width="40%">
    					...
    					<span class="minus-sign" id="rowid_<?= $model->id; ?>" onclick="turen.com.displayRows(<?= $model->id; ?>);"><?= $model->catename?></span>
    					<?php } else { ?>
    						<?=str_repeat('<span class="blank"></span>', $model->level-1)?><span class="sub-type"><?= $model->catename?></span>
    					<?php } ?>
    				</td>
    				<td width="20%" align="center">
    					<a href="<?=Url::to(['quick-move', 'type' => HelpCate::ORDER_UP_TYPE, 'kid' => $model->id, 'pid' => $model->parentid, 'orderid' => $model->orderid])?>" class="left-arrow" title="提升排序"></a>
    					<input type="text" name="orderid[]" id="orderid[]" class="inputls" value="<?= $model->orderid; ?>">
    					<a href="<?=Url::to(['quick-move', 'type' => HelpCate::ORDER_DOWN_TYPE, 'kid' => $model->id, 'pid' => $model->parentid, 'orderid' => $model->orderid])?>" class="right-arrow" title="下降排序"></a>
    				</td>
    				<td width="32%" class="action end-column">
    					<span><a href="<?= Url::to(['create', 'pid' => $model->id]) ?>">添加子帮助分类</a></span> | 
    					<span><?=$checkstr?></span> | 
